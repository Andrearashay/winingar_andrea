...
```{r}
kable(chicks_lm)
# This resulted in an error because "chicks_lm" is not a table persay but different calculated values of the comparison between weight and feed.
```
```{r}
head(trees)
```
